...
// Inspired by react-hot-toast library and Shadcn/ui toast hook
...
export function useToast(options: UseToastOptions = {}) {
  ...
  const toastFunction = useCallback(
    ({ title, description, action, duration = options.duration || 5000 }: Omit<Toast, "id">) => {
      const id = genId()
  ...
  const dismissFunction = useCallback((id: string) => {
